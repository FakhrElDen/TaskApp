...
@section('content')
    ...
    <section class="contentWrapper__cover"><!--Start contentWrapper__cover-->
        ...
        <div class="contentWrapper__cover__blogging">
            <h4 class="cap-case color-blue">Update privacy</h4>
        </div>
        <p style="color:red"> @error('privacy') {{$message}} @enderror </p>
        <form action="{{env('APP_URL')}}/updatePrivacy/{{ $privacy->id }}" method="POST">
            {{ @csrf_field() }}
            <textarea class="contentWrapper__cover__blogEdit editable" name="privacy">{!! $privacy->privacy !!}</textarea>
